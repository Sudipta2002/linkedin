...
import { useAuthentication, User } from '../../../authentication/context/AuthenticationContextProvider';
...
const Post = ({post,setPosts}:PostProps) => {
    ...
    const deletePost = async (id: number) => {
        try {
            const response =  await fetch(`${import.meta.env.VITE_API_URL}/api/v1/feed/posts/${id}`,
            {
                method: "DELETE",
                headers: {
    ...
    const editComment=async(id:number , content: string)=>{
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/comments/${id}`,{
          method: "PUT",
          headers: {
    ...
    const deleteComment = async(id:number)=>{
      try{
        ...
        setPosts((prev)=>
          prev.map((p)=>{
            if(p.id=== post.id){
              return {...p,comments:p.comments?.filter((comment)=>comment.id!==id)};
            };
    ...
    const postComment = async(e : FormEvent<HTMLFormElement>)=>{
      ...
      try {
        ...
        setPosts((prev)=>
          prev.map((p)=>{
            if(p.id === post.id){
              return {...p, comments: p.comments?[data, ...p.comments]:[data]};
    ...
    return (
    <>
    <Modal showModal={editing} setShowModal={setEditing} title='Editing a post' content={post.content} picture={post.picture} onSubmit={editPost}/>
